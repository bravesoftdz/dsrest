unit uSelfFastReportFunction;

interface
uses SysUtils, Classes, fs_iinterpreter, DateUtils, Math;

type
  TSelfFastReportFunction = class(TfsRTTIModule)
  private
...
{ TSelfFastReportFunction }



constructor TSelfFastReportFunction.Create;
begin
...
function TSelfFastReportFunction.CallMethod(Instance: TObject; ClassType: TClass;
...
function TSelfFastReportFunction.Quot(AString : String): String;
begin
  Result := QuotedStr(AString);
end;

function TSelfFastReportFunction.QuotDTAkhir(ATgl : TDateTime): string;
begin
  Result := Quot(FormatDateTime('MM/dd/yyyy 23:59:59', ATgl));
end;

function TSelfFastReportFunction.QuotDTAwal(ATgl : TDateTime): string;
begin
  Result := Quot(FormatDateTime('MM/dd/yyyy 00:00:00', ATgl));
end;

function TSelfFastReportFunction.StartOfTheMonth(ATgl : TDateTime): TDateTime;
begin
  Result := DateUtils.StartOfTheMonth(ATgl);
end;

function TSelfFastReportFunction.EndOfTheMonth(ATgl : TDateTime): TDateTime;
begin
  Result := DateUtils.EndOfTheMonth(ATgl);
end;

Function TSelfFastReportFunction.Num2Words : String;
begin
...
Function TSelfFastReportFunction.Num2Let(N:Double; aRpType:Integer):String;
Var
...
Function TSelfFastReportFunction.Num2LetEnglish(N:Double; aRpType:Integer):String;
Var
  strN, Letter, X: String;
  i, j, nX: integer;
Begin
...
function TSelfFastReportFunction.Num2LetEnglishDef(const ARupiah: Double): string;
var
  A, P: extended;
  N,I : integer;
  T   : longint;
...
initialization
  fsRTTIModules.Add(TSelfFastReportFunction);
