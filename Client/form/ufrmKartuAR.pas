...
begin
  ...
  FCDSS := ClientDataModule.ServerLaporanClient.LaporanKarAR(dtpAwal.Date, dtpAkhir.Date,lCustomer, lCabang, GetNoAR);
...
function TfrmKartuAR.GetNoAR: string;
begin
  if Trim(edNoAR.Text) = '' then
    Result := 'XXX'
  else
    Result := Trim(edNoAR.Text);
end;
